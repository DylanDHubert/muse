...
const o = class t extends i.Scene {
  ...
  update() {
    ...
    if (
      ...
      s && this.pressedKeys.size > 0)
    ) {
      const t = Array.from(this.pressedKeys),
        e = t[t.length - 1];
      e !== this.currentKey &&
        (console.log(`Key ${e} pressed, switching to that note`),
        this.switchToKey(e));
    }
    this.currentPlatform && this.currentKey && this.extendCurrentPlatform(),
      this.character.y >
        ...
        this.gameOver(),
      this.platforms.children.entries.forEach((e) => {
        const s = e;
        s.x < this.character.x - t.GAME_CONSTANTS.CLEANUP_DISTANCE &&
          s.destroy();
      });
  }
  playNote(e) {
  ...
  startNewPlatform(e) {
    ...
    this.platformStartX =
      this.character.x + t.GAME_CONSTANTS.PLATFORM_AHEAD_OFFSET;
    const i = s + t.GAME_CONSTANTS.LEVEL_HEIGHTS[e],
      r = t.GAME_CONSTANTS.LEVEL_COLORS[e];
    (this.currentPlatform = this.add.rectangle(
      this.platformStartX + t.GAME_CONSTANTS.PLATFORM_CENTER_OFFSET,
      i,
  ...
  makeCharacterJump(t) {
    ...
    if (
      (console.log(
        `Moving from ${this.currentKey || "neutral"} (level ${s}) to ${t} (level ${i})`,
      ),
      r <= 0)
    )
...
//# sourceMappingURL=index-CilSSPWt.js.map
