...
class o extends i.Scene {
  ...
  create() {
    ...
    this.add
      ...
      this.startNewPlatform("G");
  }
  update() {
    this.isGameRunning &&
      (this.character.body.setVelocityX(80),
      Object.keys(this.gameKeys).forEach((t) => {
        Phaser.Input.Keyboard.JustDown(this.gameKeys[t]) &&
          (console.log(`Key ${t} pressed!`), this.switchToKey(t));
      }),
  ...
  switchToKey(t) {
    this.currentKey && this.currentKey !== t && this.makeCharacterJump(t),
  ...
  startNewPlatform(t) {
    ...
    this.platformStartX = this.character.x;
    const i = s[t];
  ...
  extendCurrentPlatform() {
    ...
    const t = this.character.x - this.platformStartX + 50,
      e = this.platformStartX + t / 2;
    (this.currentPlatform.width = Math.max(20, t)),
  ...
  makeCharacterJump(t) {
    const e = this.character.body,
      s = this.getKeyHeight(this.currentKey);
    this.getKeyHeight(t) < s
      ? (e.setVelocityY(-400),
        console.log(`Character jumping UP from ${this.currentKey} to ${t}`))
      : (e.setVelocityY(-200),
        console.log(`Character jumping DOWN from ${this.currentKey} to ${t}`));
  }
  getKeyHeight(t) {
    const { height: e } = this.cameras.main;
    return {
...
//# sourceMappingURL=index-DYJr5kn9.js.map
