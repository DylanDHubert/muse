...
class o extends i.Scene {
  ...
  update() {
    ...
    t.setVelocityX(100), t.setDragX(0), t.setFrictionX(0);
    const e = [];
    if (
      (Object.keys(this.gameKeys).forEach((t) => {
        Phaser.Input.Keyboard.JustDown(this.gameKeys[t]) && e.push(t);
      }),
      e.length > 0)
    ) {
      const t = this.getHighestReachableKey(e);
      console.log(`Keys pressed: ${e.join(",")}, going to: ${t}`),
        this.switchToKey(t);
    }
  ...
  playNote(t) {
    console.log(`Playing note ${t} at ${this.noteFrequencies[t]}Hz`),
      this.playMusicalTone(this.noteFrequencies[t]),
  ...
  startNewPlatform(t) {
    ...
    (this.currentPlatform = this.add.rectangle(
      ...
      (this.currentKey = t),
      console.log(`Started ${t} platform at height ${i}`);
  }
  extendCurrentPlatform() {
    if (!this.currentPlatform) return;
    const t = this.character.x - this.platformStartX + 150,
      e = this.platformStartX + t / 2;
    (this.currentPlatform.width = Math.max(60, t)),
      (this.currentPlatform.x = e);
    const s = this.currentPlatform.body;
    s.setSize(this.currentPlatform.width, this.currentPlatform.height),
      s.updateFromGameObject();
  }
  endCurrentPlatform() {
  ...
  makeCharacterJump(t) {
    const e = this.character.body,
      s = this.getCurrentCharacterLevel(),
      i = this.getKeyLevel(t),
      r = i - s;
    if ((console.log(`Moving from level ${s} to ${i} (${t})`), r <= 0))
      console.log("Dropping down or staying level");
    else if (r > 2) {
      const t = s + 2,
        i = this.getLevelKey(t);
      console.log(`Too high! Can only reach level ${t} (${i})`),
        this.endCurrentPlatform(),
        this.startNewPlatform(i);
      const r = -300;
      e.setVelocityY(r);
    } else {
      const t = -200 - 50 * r;
      e.setVelocityY(t),
        console.log(`Jumping up ${r} levels with velocity ${t}`);
    }
  }
  ...
  getHighestReachableKey(t) {
    const e = this.getCurrentCharacterLevel() + 2,
      s = t.sort((t, e) => this.getKeyLevel(e) - this.getKeyLevel(t));
    for (const i of s) if (this.getKeyLevel(i) <= e) return i;
    return this.getLevelKey(e);
  }
  getKeyHeight(t) {
    const { height: e } = this.cameras.main;
    return {
...
document.addEventListener("DOMContentLoaded", () => {
  new i.Game({ ...c, parent: "game-container" });
});
//# sourceMappingURL=index-BLImDcsC.js.map
