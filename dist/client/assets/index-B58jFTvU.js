...
import { r as s } from "./phaser-y5S3hZEA.js";
!(function () {
...
class a extends i.Scene {
  ...
  create() {
    (this.camera = this.cameras.main),
      this.camera.setBackgroundColor(2236962),
      ...
      (async () => {
        try {
          const t = await fetch("/api/init");
          if (!t.ok) throw new Error(`API error: ${t.status}`);
          const e = await t.json();
          (this.count = e.count), this.updateCountText();
        } catch (t) {
          console.error("Failed to fetch initial count:", t);
        }
      })();
    const t = (t, e, s, i) => {
      const o = this.add
        ...
        .on("pointerover", () => o.setStyle({ backgroundColor: "#555555" }))
        .on("pointerout", () => o.setStyle({ backgroundColor: "#444444" }))
        .on("pointerdown", i);
      return o;
    };
    (this.incButton = t(
      0.55 * this.scale.height,
      "Increment",
      "#00ff00",
      async () => {
        try {
          const t = await fetch("/api/increment", { method: "POST" });
          if (!t.ok) throw new Error(`API error: ${t.status}`);
          const e = await t.json();
          (this.count = e.count), this.updateCountText();
        } catch (t) {
          console.error("Failed to increment count:", t);
        }
      },
    )),
      (this.decButton = t(
        0.65 * this.scale.height,
        "Decrement",
        "#ff5555",
        async () => {
          try {
            const t = await fetch("/api/decrement", { method: "POST" });
            if (!t.ok) throw new Error(`API error: ${t.status}`);
            const e = await t.json();
            (this.count = e.count), this.updateCountText();
          } catch (t) {
            console.error("Failed to decrement count:", t);
          }
        },
      )),
      (this.goButton = t(
        0.75 * this.scale.height,
        "Game Over",
        "#ffffff",
        () => {
  ...
  updateLayout(t, e) {
    if (
      (this.cameras.resize(t, e),
      this.background &&
        (this.background.setPosition(t / 2, e / 2),
        this.background.width && this.background.height))
    ) {
      const s = Math.max(t / this.background.width, e / this.background.height);
      this.background.setScale(s);
    }
    const s = Math.min(Math.min(t / 1024, e / 768), 1);
    this.countText &&
      (this.countText.setPosition(t / 2, 0.45 * e), this.countText.setScale(s)),
      this.incButton &&
        (this.incButton.setPosition(t / 2, 0.55 * e),
        this.incButton.setScale(s)),
      this.decButton &&
        (this.decButton.setPosition(t / 2, 0.65 * e),
        this.decButton.setScale(s)),
      this.goButton &&
        (this.goButton.setPosition(t / 2, 0.75 * e), this.goButton.setScale(s));
  }
  updateCountText() {
...
class r extends i.Scene {
  ...
  init() {
    (this.background = null), (this.logo = null), (this.title = null);
  }
  create() {
    this.refreshLayout(),
  ...
  refreshLayout() {
    ...
    this.cameras.resize(t, e),
      ...
      this.background.setDisplaySize(t, e);
    const s = Math.min(t / 1024, e / 768);
    this.logo || (this.logo = this.add.image(0, 0, "logo")),
      this.logo.setPosition(t / 2, 0.38 * e).setScale(s),
      this.title ||
        (this.title = this.add
          .text(0, 0, "Main Menu", {
            fontFamily: "Arial Black",
          ...
          .setOrigin(0.5)),
      this.title.setPosition(t / 2, 0.6 * e),
